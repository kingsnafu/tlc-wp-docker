...
export async function audit(config) {
  ...
  if (hasTEC) {
    ...
    const occurrences = queryScalar(
      ...
      `WHERE p.post_status='publish' AND occ.start_date >= '${cutoff}'`, config);
    ...
    console.log(`  Future occurrences (${cutoff}+): ${occurrences}`);
  ...
  console.log('\nREST API:');
  try {
    await fetchOne('/wp/v2/pages?per_page=1', config);
