...
export async function extract(config) {
  ...
  // 2. Plugin extractors (Yoast first — its meta enriches pages/posts)
  console.log('\nPlugin data:');
  const plugins = await extractPlugins(config, detected);
  const yoastMeta = plugins.yoastMeta || {};

  // 3. Taxonomies (needed for post category/tag resolution)
  console.log('\nTaxonomies:');
  const taxonomies = await extractTaxonomies(config);

  // 4. Pages
  console.log('\nPages:');
  const pages = await extractPages(config, { yoastMeta });

  // 5. Posts
  console.log('\nPosts:');
  const posts = await extractPosts(config, { yoastMeta, taxonomies: taxonomies.data.taxonomies });
