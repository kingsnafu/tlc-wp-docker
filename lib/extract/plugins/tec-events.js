...
export async function extractTecEvents(config, { mediaMap = {} } = {}) {
  ...
  for (const r of postsRows) {
    posts[parseInt(r.ID)] = { title: decodeHtmlEntities(r.post_title) || r.post_title, slug: r.post_name, description: htmlToMarkdown(r.post_content, config.wordpress.url) };
  }

  // Step 3: Get meta for all posts (including venue/organizer references)
  const metaRows = query(
    `SELECT post_id, meta_key, meta_value FROM ${prefix}postmeta ` +
    `WHERE post_id IN (${pidList}) ` +
    `AND meta_key IN ('_EventURL','_tribe_featured','_thumbnail_id','_EventVenueID','_EventOrganizerID')`,
    config,
  ...
  // Step 3b: Load venues and organizers
  const venues = loadVenues(config, prefix);
  const organizers = loadOrganizers(config, prefix);
  ...
  for (const [eid, pid] of eventIds) {
    ...
    // Resolve venue and organizer
    const venueId = m._EventVenueID ? parseInt(m._EventVenueID) : null;
    const orgId = m._EventOrganizerID ? parseInt(m._EventOrganizerID) : null;
    const venue = venueId ? venues[venueId] || null : null;
    const organizer = orgId ? organizers[orgId] || null : null;
...
/**
 * Load all published venues from the database.
 * Returns a map of venue post ID → { name, address, city, state, zip, phone }.
 */
function loadVenues(config, prefix) {
  const rows = query(
    ...
    `  MAX(CASE WHEN pm.meta_key = '_VenuePhone' THEN pm.meta_value END) AS phone ` +
    `FROM ${prefix}posts p ` +
    `LEFT JOIN ${prefix}postmeta pm ON p.ID = pm.post_id ` +
    `WHERE p.post_type = 'tribe_venue' AND p.post_status = 'publish' ` +
    `GROUP BY p.ID, p.post_title`,
    config,
    { columns: ['ID', 'post_title', 'address', 'city', 'state', 'zip', 'phone'] }
  );
  const map = {};
  for (const r of rows) {
    const v = { name: decodeHtmlEntities(r.post_title) || '' };
    if (r.address) v.address = r.address;
    if (r.city) v.city = r.city;
    if (r.state) v.state = r.state;
    if (r.zip) v.zip = r.zip;
    if (r.phone) v.phone = r.phone;
    map[parseInt(r.ID)] = v;
  }
  return map;
}
...
/**
 * Load all published organizers from the database.
 * Returns a map of organizer post ID → { name, email, phone, website }.
 */
function loadOrganizers(config, prefix) {
  const rows = query(
    `SELECT p.ID, p.post_title, ` +
    `  MAX(CASE WHEN pm.meta_key = '_OrganizerEmail' THEN pm.meta_value END) AS email, ` +
    `  MAX(CASE WHEN pm.meta_key = '_OrganizerPhone' THEN pm.meta_value END) AS phone, ` +
    `  MAX(CASE WHEN pm.meta_key = '_OrganizerWebsite' THEN pm.meta_value END) AS website ` +
    `FROM ${prefix}posts p ` +
    `LEFT JOIN ${prefix}postmeta pm ON p.ID = pm.post_id ` +
    `WHERE p.post_type = 'tribe_organizer' AND p.post_status = 'publish' ` +
    `GROUP BY p.ID, p.post_title`,
    config,
    { columns: ['ID', 'post_title', 'email', 'phone', 'website'] }
  );
  const map = {};
  for (const r of rows) {
    const o = { name: decodeHtmlEntities(r.post_title) || '' };
    if (r.email) o.email = r.email;
    if (r.phone) o.phone = r.phone;
    if (r.website) o.website = r.website;
    map[parseInt(r.ID)] = o;
  }
  return map;
}
