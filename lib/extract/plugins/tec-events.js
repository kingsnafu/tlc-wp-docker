 ...
 */
export async function extractTecEvents(config) {
  ...
  const eventRows = query(
    ...
    `WHERE occ.start_date >= '2026-01-01' ` +
  ...
  for (const r of postsRows) {
    posts[parseInt(r.ID)] = { title: r.post_title, slug: r.post_name, description: r.post_content };
  }
  ...
  // Step 4: Get featured image URLs
  const thumbIds = Object.values(meta)
    .map(m => m._thumbnail_id)
    .filter(id => id && id !== 'NULL');
  const imageUrls = {};
  if (thumbIds.length) {
    const imgRows = query(
      `SELECT ID, guid FROM ${prefix}posts ` +
      `WHERE ID IN (${thumbIds.join(',')}) AND post_type = 'attachment'`,
      config,
      { columns: ['ID', 'guid'] }
    );
    for (const r of imgRows) {
      imageUrls[parseInt(r.ID)] = r.guid;
    }
  }
  ...
  for (const [eid, pid] of eventIds) {
    ...
    const imgUrl = thumbId ? imageUrls[thumbId] || null : null;
